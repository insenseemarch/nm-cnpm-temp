...
const AddEventForm = ({ onSubmit, onClose }) => {
  ...
  // NOTE: The `value` field MUST match backend's expected eventType values
  // to avoid everything falling back to "Khác".
  // Adjust these strings if backend enum/contract changes.
  const eventTypes = [
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        // onSubmit should return true/false (or truthy) based on API success
        ...
        if (result !== false) {
          ...
          showNotification('Tạo sự kiện thất bại. Vui lòng thử lại.', 'warning');
        }
      } catch (err) {
        console.error('Add event failed in form:', err);
        showNotification('Tạo sự kiện thất bại. Vui lòng thử lại.', 'warning');
      }
    }
  };
