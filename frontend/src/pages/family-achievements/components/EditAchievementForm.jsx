...
const EditAchievementForm = ({ achievement, onSubmit, onCancel }) => {
  ...
  // Populate form with achievement data when component mounts
  useEffect(() => {
    if (achievement) {
      setFormData({
        name: achievement.name || '',
        title: achievement.title || achievement.description?.split('.')[0] || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Send updated data with ID to parent handler
      await onSubmit({
        ...formData,
        id: achievement?.id
      });
      ...
      // Close notification and form after 300ms to allow user to see the success message
      setTimeout(() => {
        closeNotification();
        onCancel();
      }, 300);
  ...
  return (
    <>
      <motion.div
        className="fixed inset-0 bg-background/80 backdrop-blur-md flex items-center justify-center z-[70] px-4"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
                ...
                >
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="text-sm text-white/50 group-hover:text-white/80 transition-colors">
                      <span className="font-semibold">Thêm ảnh mới</span>
                      ...
                      >
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/80 to-transparent">
                          <p className="text-xs text-white/80 truncate px-1">
                            {typeof file === 'string' ? 'Ảnh hiện tại' : file.name}
                          </p>
                        </div>
                      </motion.div>
                    ))}
